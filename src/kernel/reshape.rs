...
pub fn reshape_columns_any(mask: &BooleanArray, stride: usize) -> BooleanArray {
...
pub fn reshape_columns_none(mask: &BooleanArray, stride: usize) -> BooleanArray {
...
/// Shrink mask by and-not-ing stride-length stretches of values together.
pub fn reshape_rows_none(mask: &BooleanArray, stride: usize) -> BooleanArray {
    let mut result = vec![true].repeat(mask.len() / stride);
    for i in 0..mask.len() {
        result[i % stride] &= !mask.value(i)
    }
